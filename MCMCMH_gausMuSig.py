...
def priorUni(a, b):    return lambda x : uniform(a, b, x)
def priorGaus(a, b):   return lambda x : gaussian(a, b, x)
def likelihoodGaus(x): return lambda a, b: gaussian(a, b, x)
...
#plots
    ...
    ax0.axvline(x=np.average(dataPDF), ymin=0, ymax=1, color="darkviolet", linestyle="--")
    ...
    if "sig" in traceOption:
        ...
        rejPlot    = mcmcSigRej
        traceTitle = "sigma"
    ax2.plot(iteration, tracePlot, alpha=1.0, color="black", drawstyle="steps-post", zorder=0)
    ax2.scatter(iterAcc, accPlot, alpha=0.8, color="blue", marker="o", s=20, zorder=2)
    ax2.scatter(iterRej, rejPlot, alpha=0.8, color="red", marker="x", s=30, zorder=1)
    ax2.axhline(y=np.average(accPlot), xmin=0, xmax=1, color="darkviolet", linewidth=3,\
                linestyle="--", zorder=3)
    ...
    #plot 3
    res3 = ax3.hexbin(mcmcMuAcc, mcmcSigAcc, cmap="jet", gridsize=max(10, int(np.sqrt(mcmcN)/2.0)))
    fig.colorbar(res3, ax=ax3).ax.zorder = -1
    plt.sca(ax3)
    ax3.axvline(x=np.average(mcmcMuAcc),  ymin=0,ymax=1,color="darkviolet",linewidth=3,linestyle="--")
    ax3.axhline(y=np.average(mcmcSigAcc), xmin=0,xmax=1,color="darkviolet",linewidth=3,linestyle="--")
...
#plot autocorrelation:
    if plotAutoCorr == True:
        ...
        mcmcSigAcc_autoCorr = computerAutoCorr(mcmcSigAcc, maxLag=maxLag)
        ...
        ax1.plot(range(maxLag), mcmcSigAcc_autoCorr, alpha=1.0, color="black", drawstyle="steps-post")
        ax1.set_title("Autocorrelation of sigma", fontsize=24, y=1.03)
        ax1.set_xlabel("trace step lag", fontsize=18)
        ax1.set_ylabel("amplitude (norm to std)", fontsize=18)
        ax1.set_xlim(0, maxLag)
        ax1.set_ylim(-0.15, 1.05)
        ax1.axhline(y=0, xmin=0, xmax=maxLag, color="grey", linewidth=1, linestyle="-", alpha=0.5) 

        exepath = os.path.dirname(os.path.abspath(__file__))
        filenameFigAuto = exepath + "/gausMuSigAuto.png"
        gs.tight_layout(fig)
